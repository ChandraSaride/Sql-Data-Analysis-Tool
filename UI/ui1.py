...
def seechangeiscentroid(k_val):
    ...
    centroid_shift = [0] * k
    for i in range(k):
        centroid_shift[i] = math.sqrt(
            (abs(x_list3[i][0] - x_list3[i][1]) ** 2) + (abs(y_list3[i][0] - y_list3[i][1]) ** 2) + (
                        abs(z_list3[i][0] - z_list3[i][1]) ** 2))  # / \
...
B5 = Button(top, text="View Clusters Full Data (validation)", command=lambda: retrievevalidationdata(E21.get()),
            state="disabled")
...
B7 = Button(top, text="View Star Rating Prediction error per cluster", command=lambda: showpredictionerror(E21.get()),
            state="disabled")
